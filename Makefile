...
# Auto-detect host uplink interface (e.g., enp0s3)
...
# -----------------------------------------
# 1) Bring up VPC1 with public/private subnets
# -----------------------------------------
up:
...
# -----------------------------------------
# 2) Enable NAT for VPC1 public subnet
# -----------------------------------------
nat:
	sudo ./vpcctl.sh nat-enable vpc1 10.200.1.0/24 $(IFACE)

# -----------------------------------------
# 3) Apply Security Group–like rules
# -----------------------------------------
policy:
...
# -----------------------------------------
# 4) Create a second VPC and peer PUBLIC subnets
# -----------------------------------------
peer:
	sudo ./vpcctl.sh create vpc2
	sudo ./vpcctl.sh add-subnet vpc2 public 10.201.1.0/24
	sudo ./vpcctl.sh nat-enable vpc2 10.201.1.0/24 $(IFACE)
	sudo ./vpcctl.sh peer vpc1 vpc2 10.200.1.0/24 10.201.1.0/24 $(IFACE)

# -----------------------------------------
# 5) Run a quick connectivity test suite
# -----------------------------------------
test:
	@echo "== Intra-VPC connectivity =="; \
	sudo ip netns exec vpc-vpc1-public  ping -c1 -W1 10.200.2.2 | sed 's/^/[vpc1]/' ; \
	sudo ip netns exec vpc-vpc1-private ping -c1 -W1 10.200.1.2 | sed 's/^/[vpc1]/' ; \
	echo "== NAT on public =="; \
	...
	sudo ip netns exec vpc-vpc1-public bash -lc 'nohup python3 -m http.server 8080 --bind 10.200.1.2 >/dev/null 2>&1 &' ; \
	sleep 1 ; \
	curl -sI http://10.200.1.2:8080 | head -n1 ; \
	timeout 2 bash -lc 'exec 3<>/dev/tcp/10.200.1.2/22' || echo "PORT22_BLOCKED ✅"
...
# Full demo chain (bring up, NAT, policy, tests)
demo: up nat policy test

# Remove leftovers if you ever need a hard reset
nuke: down
	@echo "Cleaning residual routes and rules..."
	@sudo ip route del 10.200.1.0/24 2>/dev/null || true
